...
# -----------------------------
# Tail: Generalized-Gamma (Stacy) PDF
# -----------------------------
def generalized_gamma_stacy_pdf(S):
    d = 2.38342
    p = 1.98133
    a = 1.01145
    coeff = p / (a**d * gamma(d / p))
    return coeff * (S + 1e-12)**(d - 1) * np.exp(-((S + 1e-12) / a)**p)
...
def hybrid_psi_model(S, p):
    C_tail = 1.0
    body = psi_body(S, p)
    tail = C_tail * generalized_gamma_stacy_pdf(S)
    cutoff = 2.25
    width = 0.25
...
# -----------------------------
# Initial Parameter Guesses (updated)
# -----------------------------
p0 = [
...
pS_vals = C * (psi_vals / generalized_gamma_stacy_pdf(S_plot))
